...
def train(train_images_path, train_images_label, val_images_path=None, val_images_label=None, classes=5, epochs=10, batch_size=32,
          lr=0.001, lrf=0.01, weight='./init/vit_base_patch16_224_in21k.pth', freeze=False, device='cuda:0', val=True, init=True):
    ...
    for epoch in range(epochs):
        ...
        f = open('./res_dir/train_res/vit_res.txt', 'a')
        ...
        if best_acc < train_acc:
            best_acc = train_acc
            best_epoch = epoch
            torch.save(model.state_dict(), './res_dir/weights/best_vit_model.pth')
    f1 = open('./res_dir/best_res/best_vit.txt', 'a')
...
def predict(images_path, images_label, num_class=5, batch_size=8):
    ...
    model_weight_path = './res_dir/weights/best_vit_model.pth'
    model = vit_base_patch16_224_in21k(num_classes=num_class, has_logits=False).to(device)
    model.load_state_dict(torch.load(model_weight_path, map_location=device))
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    score = cal_m(model=model, data_loader=loader, device=device, alo='vit')
    ...
    metrics_df.to_csv('./res_dir/label/vit_metrics.csv', header=True, index=False)
    ...
    confusion_mat_df.to_csv('./res_dir/label/vit_confusion_matrix.csv')    
    
    pd.DataFrame(np.array(score)).to_csv('./res_dir/label/vit' + '.csv', header=None, index=None)
    return score
